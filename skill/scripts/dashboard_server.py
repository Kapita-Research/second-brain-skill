...
TASK_LOG_LABEL = {"done": "Done", "cancelled": "Cancelled", "in-progress": "Started",
                  ...
                  "not-started": "Reopened", "delegated-out": "Delegated out"}
# Task statuses that take a task off the vault owner's plate — no overdue flag, not
# counted as open, hidden by default. `delegated-out` = someone else owns the work AND
# its supervision. Keep in sync with CLOSED_STATUS in the browser code below.
        ...
        if t == "task":
            ...
            tasks.append({
                ...
                "overdue": bool(DATE_RE.match(due)) and due < today
                           and status not in CLOSED_TASK_STATUS,
...
:root{
  ...
  --stat-handed-bg:#CDE7E3;--stat-handed-fg:#0B2F2B;
...
:root[data-theme=dark]{
  ...
  --stat-handed-bg:#2A4642;--stat-handed-fg:#C7E5E0;
}
...
.s-delegated-out{background:var(--stat-handed-bg);color:var(--stat-handed-fg)}
...
// Task statuses that take a task off the vault owner's plate: not counted as open,
// never flagged overdue, hidden by default. Mirrors CLOSED_TASK_STATUS on the server
// — keep the two in sync.
const CLOSED_STATUS = ["done", "cancelled", "delegated-out"];
...
function renderTabs(){
  ...
  for (const d of defs){
    ...
    if (d.id !== "overview"){
      ...
      const open = base.filter(t => !CLOSED_STATUS.includes(t.status)).length;
...
function refreshOverdueFlag(t){
  ...
  t.overdue = ISO.test(t.due) && t.due < data.today
              && !CLOSED_STATUS.includes(t.status);
}
...
function cellFor(c, t){
  switch (c.id){
    ...
    case "days_left": {
      const n = daysLeft(t);
      return el("td", {class: n !== "" && n < 0 && !CLOSED_STATUS.includes(t.status) ? "due-over" : ""},
                n === "" ? "—" : String(n));
    }
...
async function refresh(){
  ...
  try {
    ...
    if (!state._init){
      state._init = true;
      // first run: hide closed statuses by default (clearable like any filter)
      if (state.colf.status === undefined)
        state.colf.status = data.vocab.task.filter(s => !CLOSED_STATUS.includes(s));
      saveState();
    }
