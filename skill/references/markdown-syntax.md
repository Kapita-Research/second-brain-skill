...
Use `[[wikilinks]]` for internal vault links. Use standard `[text](url)` for external URLs.

> **Renaming is not automatically safe.** Obsidian rewrites inbound `[[wikilinks]]` only when **the app itself** performs the rename — in-app, or `obsidian rename`. A rename done with the file tools orphans every inbound link, because a wikilink resolves by **basename** and there is nothing left to resolve. *Moving* a note is always safe (the folder was never in the link). Full rule and the tested evidence: `vault-structure.md`.
