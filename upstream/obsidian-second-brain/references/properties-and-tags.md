...
This is the **schema backbone** of the vault. Consistent properties are what make a Markdown vault queryable instead of a pile of text. **Use only the values defined here.** Do not invent new property names or status values per note — if something genuinely doesn't fit, ask the user before extending the vocabulary.
...
| `last-contact` | optional | `YYYY-MM-DD` — set when filing an interaction with this person |
...
**Tasks:** `not-started` → `in-progress` → `in-review` → `done` (plus `on-hold`, `cancelled`). `in-review` = the assignee has delivered and is waiting on someone else's review or feedback — it goes back to `in-progress` if changes are asked for, or forward to `done` when approved. Distinct from `on-hold`, where nobody is working and nobody owes an answer
